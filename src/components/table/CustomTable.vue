...
<template>
  ...
  <section>
    <el-table :data="props.tableData" height="585px" v-bind="props.tableOptions?.table?.attrs"
      ...
      <el-table-column align="left" :formatter="cellFormatter" v-for="(item, index) in columns" :key="index"
      ...
      <el-table-column v-if="props.tableOptions?.column?.buttonGroup" label="操作" align="center"
        :min-width="`${(props.tableOptions?.column?.buttonGroup?.length ?? 1) * 30}px`">
    ...
    <el-divider v-if="props.tableOptions?.pagination?.visible ?? false"></el-divider>
